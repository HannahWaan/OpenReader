...
class ThemeState {
  ...
  final double fontSize, lineHeight, marginH;
  final String fontFamily;

  const ThemeState({this.themeMode = ThemeMode.system, this.readerTheme = ReaderThemeType.light,
    this.fontSize = 18.0, this.fontFamily = 'Merriweather', this.lineHeight = 1.7, this.marginH = 24.0});
  ...
  ThemeState copyWith({ThemeMode? themeMode, ReaderThemeType? readerTheme,
    double? fontSize, String? fontFamily, double? lineHeight, double? marginH}) => ThemeState(
    themeMode: themeMode ?? this.themeMode, readerTheme: readerTheme ?? this.readerTheme,
    fontSize: fontSize ?? this.fontSize, fontFamily: fontFamily ?? this.fontFamily,
    lineHeight: lineHeight ?? this.lineHeight, marginH: marginH ?? this.marginH);
}
...
class ThemeNotifier extends StateNotifier<ThemeState> {
  ...
  Future<void> _load() async {
    ...
    state = ThemeState(themeMode: ThemeMode.values[p.getInt('themeMode') ?? 0],
      readerTheme: ReaderThemeType.values[p.getInt('readerTheme') ?? 0],
      fontSize: p.getDouble('fontSize') ?? 18.0, fontFamily: p.getString('fontFamily') ?? 'Merriweather',
      lineHeight: p.getDouble('lineHeight') ?? 1.7, marginH: p.getDouble('marginH') ?? 24.0);
  }
